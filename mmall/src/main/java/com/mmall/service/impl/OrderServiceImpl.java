...
public class OrderServiceImpl implements IOrderService {
    ...
    private Order assembleOrder(Integer userId,Integer shippingId,BigDecimal payment){
        ...
        order.setPostage(0);
        order.setPaymentType(Const.PaymentTypeEnum.ONLINE_PAY.getCode());   //在线支付or线下支付
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path){
        ...
        for (OrderItem orderItem:orderItemList){
            GoodsDetail goods = GoodsDetail.newInstance(orderItem.getProductId().toString(), orderItem.getProductName(),
                    ...
                    orderItem.getQuantity());   //产品id，产品名称，产品单价（单位是分），产品数量
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                //到目前为止，我们做的是，根据web订单号生成路径，路径会生成二维码，按照路径和新的文件名传到ftp服务器上
                //文件名获取后，组装url
    ...
    public ServerResponse aliCallback2(Map<String,String> params){
        ...
        if(order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()){//订单状态
            return ServerResponse.createBySuccess("支付宝重复调用");
        }
        if(Const.AlipayCallback.TRADE_SUCCESS.equals(tradeStatus)){     //如果成功，则更新订单状态为已付款
